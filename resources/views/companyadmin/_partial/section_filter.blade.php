    ...
    <div class="form-group">
        ...
        <div>
            <select name="office" id="office" class="form-control">
                @if ($officeList != null)
                    <option value="{{ $officeList[0]['masterOfficeId'] }}" data-isRetail="-1" data-isAdmin="6"
                        class="optionGroup">{{ __('All Entities') }}</option>

                    @foreach ($officeList as $key => $office)
                        @if ($key == 0)
                            <option value="{{ $officeList[0]['masterOfficeId'] }}" data-isRetail="0" data-isAdmin="4"
                                class="optionGroup">&nbsp;&nbsp;&nbsp;&nbsp;{{ __('All Companies') }}</option>
                        @endif
                    ...
                    <option value="{{ $officeList[0]['masterOfficeId'] }}" data-isRetail="-1" data-isAdmin="1"
                        class="optionGroup">&nbsp;&nbsp;&nbsp;&nbsp;{{ __('All Pumps') }}</option>

                    {{-- Wholesale --}}
                    @foreach ($officeList as $key => $office)
                        @if ($key == 0)
                            <option value="{{ $office['masterOfficeId'] }}" data-isRetail="0" data-isAdmin="3"
                                class="optionGroup">
                                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{{ __('WholeSale Pumps') }}</option>
                        @endif
