        ...
        <div id="chartOneData" class="d-none w-100" style="height: 200px; overflow-y:auto">
            ...
            <table id="chartOneDataTable" class="table-striped  display nowrap stattab" style="width:100%">
                ...
                <thead>
                    <tr>
                        <th colspan="3"
                            style="text-align:center; background: rgba(81, 173, 85,1); border-bottom:1px solid rgb(4, 27, 10)">
                            {{ __('Sales-Expense Summary Data') }}</th>
                    </tr>
                    <tr
                        style="text-align:center; background: rgba(81, 173, 85,1); border-bottom:1px solid rgb(4, 27, 10)">
                        <th scope="col" style="width:50%;" class="text-left">{{ __('Date') }}</th>
                        <th scope="col" style="width:25%;border-left:1px dashed #aaa" class="text-right">
                            {{ __('Sales') }}</th>
                        <th scope="col" style="width:25%;border-left:1px dashed #aaa" class="text-right">
                            {{ __('Expense') }}</th>
                    </tr>
                    ...
                    @if (isset($labels))
                        @for ($i = 0; $i < count($labels); $i++)
                            ...
                            <tr>
                                ...
                                <td scope="col" style="width:25%;border-left:1px dashed #aaa"
                                class="text-right">{{ __(number_format($data_sales[$i], 2, '.', '')) }}</td>
                                <td scope="col" valign='middle' style="width:25%;border-left:1px dashed #aaa"
                                    class="text-right">{{ __(number_format($data_expense[$i], 2, '.', '')) }}</td>
                            </tr>
                        @endfor
                    @endif
                    <tr style="">
                        <td scope="col" style="width:50%; text-align:right; ">{{ __('Total') }}</td>
                        <td scope="col" style="width:25%;; text-align:right;border-left:1px dashed #aaa"
                        class="text-right">{{ __(number_format($sales, 2, '.', '')) }}</td>
                        <td scope="col" valign='middle'
                            style="width:25%;; text-align:right;border-left:1px dashed #aaa"
                            class="text-right">{{ __(number_format($expense, 2, '.', '')) }}</td>
                    </tr>
...
<script>
 ...
 //console.log(sales);
   ...
   // console.log(labelsChart1.length);
    var c_type = 'line';
    ...
    var dataChart1 = {
        ...
        datasets: [{
                label: '{{__('Sales')}}',
                backgroundColor: c_type == 'bar' ? 'rgb(57, 161, 0)' : 'rgb(57, 161, 0,0.5)',
            ...
            {
                label: '{{__('Expense')}}',
                backgroundColor: c_type == 'bar' ? 'rgb(255, 99, 82)' : 'rgb(255, 99, 82,0.5)',
    ...
    //draw average liine inside chart
...
<script>
    $(document).ready(function() {
        ...
        $('#graphOnepdf').click(function() {
            ...
            //  alert(officeId);
            $.ajax({
                url: '{{ route('companyadmin.chart1_pdf') }}',
                type: 'POST',
                xhrFields: {
                    responseType: 'blob'
                },
        ...
        $('#graphOneexcel').click(function() {
            ...
            var fileName = officeName+'_sales-expense-summary_'+ fromDate+'_to_'+ toDate +'.xlsx';

            //  alert(officeId);
            $.ajax({
                url: '{{ route('companyadmin.chart1_excel') }}',
                type: 'POST',
                xhrFields: {
                    responseType: 'blob'
                },
