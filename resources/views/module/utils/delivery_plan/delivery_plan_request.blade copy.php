...
<form id="formSavePlan">
    <div class="row justify-content-center align-items-center mb-4">
        ...
        <div class="col-12  gap-5  d-flex flex-row align-items-center justify-content-end mt-2 overflow-hidden ">
            <div class="btn-box animated-shine " onclick="toggleRequestPanel(this);"
            ...
            <div class="btn-box btn-block animated-shine update-route  " onclick="updateRoute(this);"
                title="{{ __('Optimize route') }}">

                <i class="fa fa-bullseye"></i>
                <div>{{ __('Optimize Route') }}</div>
            </div>
...
@if (isset($response))
    <script>
            ...
            $.ajax({
                ...
                if (!data.status) {
                    ...
                    setTimeout(() => {
                        ...
                        var XXExpectedReturnTime = newList[newList.length - 1]
                        .estimatedDeliveryTime;
                        var ExpectedReturnTime = datetimeLocal(XXExpectedReturnTime);
        ...
        //console.log(ExpectedReturnTime);
        $('#ExpectedReturnTime').val(ExpectedReturnTime);
        $('#ModifiedExpectedReturnTime').val(ExpectedReturnTime);
       ...
       async function calculateSummary() {
            ...
            var strOne = `<div class="rowHeader">` +
                `<div title="${limitWarning}">Total requirements for  suggested ${(newList.length - 2)} pump(s) is ` +
                `:<span class="${styleCheck}"> ${ parseFloat(addOne.atDeliveryRequirement).toFixed(0)} ltr</span></div>` +
                `</div>`;
                    ...
                    "render": function(data, type, full, meta) {
                        ...
                        if (data.hasOwnProperty('DeliveryPlanId')) {
                            ...
                            if (data.hasOwnProperty('ApproveStatus')) {
                                ...
                                var this_status =
                                    ` <div class="td_box d-flex align-items-center justify-content-center">` +
                                    `<a href="javascript:" ` +
