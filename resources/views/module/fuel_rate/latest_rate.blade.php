<div class="modal-dialog modal-lg  modal-dialog-centered mt-0">
    ...
    <div class="modal-content bg-info">
        <div class="modal-header">
            <h4 class="modal-title text-light">{{ __('Fuel Rate') }} </h4>
                    ...
                    <section class="content">
                        <div class="rounded card p-3 bg-white shadow min-h-100">
                            <div class="row mx-auto">
                                ...
                                <h3 class="card-title  "> {{__('Fuel Rates for')}} {{ $office['officeName'] }} as on
                                    {{ date('d-m-Y') }}</h3>
                                        ...
                                        <tbody>
                                            ...
                                            @forelse ($latest_rate as $item)
                                                <tr>
                                                    <td>
                                                        {{ $item['productName'] }}
                                                        <input type="text" class="sr-only" name="productTypeId[]" value="{{$item['productTypeId']}}">
                                                        <input type="text" class="sr-only" name="currentRate[]" value="{{$item['fuelRate']}}">
                                                    ...
                                                    <td style="border-bottom:2px dashed #000!important;">
                                                        ...
                                                        onInput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"
                                                         name="rate[]" value="{{ number_format($item['fuelRate'],2,'.','') }}">
                                                    </td>
                                                </tr>
